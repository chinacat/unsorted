...
(define (register-program proc)
  (set! *program* proc))

(define-syntax (query stx)
  (syntax-case stx ()
    [(_ term)
     #'(let-values ([(new-term frame-size) (annotate-clause 'term)])
         (*program* ((make-copy-term new-term) (make-vector frame-size))))]))

(define-syntax (program stx)
  (syntax-case stx ()
    [(_ head)
     #'(let-values ([(new-term frame-size) (annotate-clause 'head)])
         (register-program (let ([unify-head (make-unify-head new-term)])
                             (lambda (arg)
