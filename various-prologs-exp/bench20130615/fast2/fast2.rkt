...
; fast2 -- enhance the call / clause entry code to specialize on the arity of the predicate
;          (0 args, 1 arg, 2 args, 3 args, 'many' args)
...
(define (generate-fact/0 head frame-size)
  (lambda (other-clauses sk fk)
...
(define (generate-fact/1 head frame-size)
  (let ([unify-arg1 (make-unify-head (cadr head))])
    (lambda (other-clauses sk fk arg1)
      (let ([frame (make-vector frame-size)])
        (let ([old-trail *trail*])
          (cond
            [(unify-arg1 arg1 frame)
             (if (pair? other-clauses)
                 (sk (lambda ()
            ...
            [(pair? other-clauses)
             (reset-trail old-trail)
             ((clause-proc (car other-clauses)) (cdr other-clauses) sk fk arg1)]
            [else
             (fk)]))))))

(define (generate-fact/2 head frame-size)
  (let ([unify-arg1 (make-unify-head (cadr head))]
        [unify-arg2 (make-unify-head (caddr head))])
    (lambda (other-clauses sk fk arg1 arg2)
      (let ([frame (make-vector frame-size)])
        (let ([old-trail *trail*])
          (cond
            [(and (unify-arg1 arg1 frame) (unify-arg2 arg2 frame))
             (if (pair? other-clauses)
                 (sk (lambda ()
                       (reset-trail old-trail)
                       ((clause-proc (car other-clauses))
            ...
            [(pair? other-clauses)
             (reset-trail old-trail)
             ((clause-proc (car other-clauses)) (cdr other-clauses) sk fk arg1 arg2)]
            [else
             (fk)]))))))

(define (generate-fact/3 head frame-size)
  (let ([unify-arg1 (make-unify-head (cadr head))]
        [unify-arg2 (make-unify-head (caddr head))]
        [unify-arg3 (make-unify-head (cadddr head))])
    (lambda (other-clauses sk fk arg1 arg2 arg3)
      (let ([frame (make-vector frame-size)])
        (let ([old-trail *trail*])
          (cond
            [(and (unify-arg1 arg1 frame) (unify-arg2 arg2 frame) (unify-arg3 arg3 frame))
             (if (pair? other-clauses)
                 (sk (lambda ()
                       (reset-trail old-trail)
                       ((clause-proc (car other-clauses))
            ...
            [(pair? other-clauses)
             (reset-trail old-trail)
             ((clause-proc (car other-clauses)) (cdr other-clauses) sk fk arg1 arg2 arg3)]
            [else
             (fk)]))))))

(define (generate-fact/many head frame-size)
...
(define (generate-fact head frame-size)
  (case (length (cdr head))
    [(0) (generate-fact/0 head frame-size)]
    [(1) (generate-fact/1 head frame-size)]
    [(2) (generate-fact/2 head frame-size)]
    [(3) (generate-fact/3 head frame-size)]
    [else (generate-fact/many head frame-size)]))

(define (generate-rule/0 head body frame-size)
  (let ([prove-body (analyze-goal-lst body #f)])
    (lambda (other-clauses sk fk)
      (let ([frame (make-vector frame-size)])
        (let ([old-trail *trail*])
          (if (pair? other-clauses)
              (prove-body frame sk
                          (lambda ()
                            (reset-trail old-trail)
                            ((clause-proc (car other-clauses)) (cdr other-clauses) sk fk))
                          fk)
              (prove-body frame sk fk fk)))))))

(define (generate-rule/1 head body frame-size)
  (let ([unify-arg1 (make-unify-head (cadr head))]
...
(define (generate-rule/2 head body frame-size)
  (let ([unify-arg1 (make-unify-head (cadr head))]
        [unify-arg2 (make-unify-head (caddr head))]
        [prove-body (analyze-goal-lst body #f)])
    (lambda (other-clauses sk fk arg1 arg2)
...
(define (generate-rule/3 head body frame-size)
  (let ([unify-arg1 (make-unify-head (cadr head))]
        [unify-arg2 (make-unify-head (caddr head))]
        [unify-arg3 (make-unify-head (cadddr head))]
        [prove-body (analyze-goal-lst body #f)])
...
(define (generate-rule/many head body frame-size)
  (let ([unify-head (make-unify-head (cdr head))]
        [prove-body (analyze-goal-lst body #f)])
    (lambda (other-clauses sk fk arg)
      (let ([frame (make-vector frame-size)])
...
(define (generate-rule head body frame-size)
  (case (length (cdr head))
    [(0) (generate-rule/0 head body frame-size)]
    [(1) (generate-rule/1 head body frame-size)]
    [(2) (generate-rule/2 head body frame-size)]
    [(3) (generate-rule/3 head body frame-size)]
    [else (generate-rule/many head body frame-size)]))
...
(define (analyze-goal/0 goal top-level?)
      ...
      (let ([clauses (get-clauses predicate-name)])
        (cond
          ...
          [(procedure? clauses)
           (clauses sk fk)]
          [else
           ((clause-proc (car clauses)) (cdr clauses) sk fk)])))))

(define (analyze-goal/1 goal top-level?)
  (let ([predicate-name (car goal)]
        [arg1 (cadr goal)])
    (let ([copy-arg1 (if top-level?
                         (lambda (frame) arg1)
                         (make-copy-term arg1))])
      (lambda (frame sk fk)
        (let ([clauses (get-clauses predicate-name)])
          (cond
            [(null? clauses)
             (error 'prove-goal "predicate ~a not defined" predicate-name)]
            [(procedure? clauses)
             (clauses sk fk (copy-arg1 frame))]
            [else
             ((clause-proc (car clauses)) (cdr clauses) sk fk (copy-arg1 frame))]))))))

(define (analyze-goal/2 goal top-level?)
  (let ([predicate-name (car goal)]
        [arg1 (cadr goal)]
        [arg2 (caddr goal)])
    (let ([copy-arg1 (if top-level?
        ...
        (let ([clauses (get-clauses predicate-name)])
          (cond
            [(null? clauses)
             (error 'prove-goal "predicate ~a not defined" predicate-name)]
            [(procedure? clauses)
             (clauses sk fk (copy-arg1 frame) (copy-arg2 frame))]
            [else
             ((clause-proc (car clauses)) (cdr clauses) sk fk 
                                          (copy-arg1 frame)
                                          (copy-arg2 frame))]))))))

(define (analyze-goal/3 goal top-level?)
  (let ([predicate-name (car goal)]
        [arg1 (cadr goal)]
        [arg2 (caddr goal)]
        [arg3 (cadddr goal)])
        ...
        (let ([clauses (get-clauses predicate-name)])
          (cond
            [(null? clauses)
             (error 'prove-goal "predicate ~a not defined" predicate-name)]
            [(procedure? clauses)
             (clauses sk fk (copy-arg1 frame) (copy-arg2 frame) (copy-arg3 frame))]
            [else
             ((clause-proc (car clauses)) (cdr clauses) sk fk 
                                          (copy-arg1 frame)
                                          (copy-arg2 frame)
...
(define (analyze-goal/many goal top-level?)
  (let ([predicate-name (car goal)]
        [arg (cdr goal)])
...
(define (analyze-goal goal top-level?)
  (case (length (cdr goal))
    [(0) (analyze-goal/0 goal top-level?)]
    [(1) (analyze-goal/1 goal top-level?)]
    [(2) (analyze-goal/2 goal top-level?)]
    [(3) (analyze-goal/3 goal top-level?)]
    [else (analyze-goal/many goal top-level?)]))
