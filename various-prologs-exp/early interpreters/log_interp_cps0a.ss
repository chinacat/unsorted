...
; log_interp_cps0a: alternative fork -- count the calls to prove
;       curiously, this yields a much smaller number than mentioned by Norvig in PAIP, section 11.4

(require (only-in mzlib/compat
                  getprop
                  putprop))

(define logical-inference-count 0)
(define (reset-lif) (set! logical-inference-count 0))
(define (incr-lif) (set! logical-inference-count (+ logical-inference-count 1)))
...
; TO CPS
(define (search-choices goal clauses bindings other-goals)
  (define (inner lst)
    (if (null? lst)
        fail
        (let* ([new-clause (rename-variables (car lst))]
               [test (prove-all (append (clause-body new-clause) other-goals)
...
(define (prove goal bindings other-goals)
  ...
  (let ([clauses (get-clauses (predicate goal))])
    (if (pair? clauses)
        (search-choices goal clauses bindings other-goals)
