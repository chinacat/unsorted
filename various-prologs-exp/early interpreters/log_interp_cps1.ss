...
; TO CPS
(define (prove goal bindings other-goals k)
  (let ([clauses (get-clauses (predicate goal))])
    (if (pair? clauses)
        (search-choices goal clauses bindings other-goals k)
        (clauses (cdr goal) bindings other-goals k))))

; TO CPS
(define (prove-all goals bindings k)
  (cond
    [(eq? bindings fail)
     (k fail)]
    [(null? goals)
     (k bindings)]
    [else
     (prove (car goals) bindings (cdr goals) k)]))
...
(define (show-prolog-vars vars bindings other-goals k)
  (if (null? vars)
      (printf "~nYes.")
      (for-each (lambda (var)
  ...
  (if (continue?)
      (k fail)
      (prove-all other-goals bindings k)))
