...
; TO CPS
(trace-define (search-choices goal clauses bindings other-goals cut-k)
  (trace-define (inner lst k)
    (if (null? lst)
        (k fail)
        (let ([new-clause (rename-variables (car lst))])
          (prove-all (append (clause-body new-clause) other-goals)
                     ...
                     cut-k
                     (trace-lambda inner-k (test)
                       (printf "[TRACE] inner-k~n")
                       (printf "[TRACE] lst = ~a~n" lst)
                       (printf "[TRACE] other-goals = ~a~n" other-goals)
                       (printf "[TRACE]~n")
                       (if (eq? fail test)
                           (begin (printf "inner/here!, lst = ~a~n" lst)
                                  (inner (cdr lst) k))
                           (begin (printf "in search-choices, caught a failure~n")
                                  (if (eq? k cut-k)
                                      (printf "cut-k = k~n")
                                      (printf "cut-k != k~n"))
                                  (k test))))))))
...
; TO CPS
(trace-define (prove goal bindings other-goals cut-k k)
  (if (eq? goal '!)
...
; TO CPS
(trace-define (prove-all goals bindings cut-k k)
...
(trace-define (show-prolog-vars vars bindings other-goals cut-k k)
  (if (null? vars)
      (printf "~nYes.")
      (for-each (lambda (var)
...
; TO-CPS
(trace-define (top-level-prove goals)
  (let ([k (lambda (_)
             (printf "~nNo.")
             (void))])
