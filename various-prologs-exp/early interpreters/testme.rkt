...
(define (try-each goal clauses lenv fg sk fk)
  (cond
    [(null? (cdr clauses))
     (try-goal goal
               (car clauses)
    ...
    [else
     (try-goal goal
               ...
               fg
               sk
               (lambda () (try-each goal (cdr clauses) lenv fg sk fk))
               fk)]))

(define (try-goal goal clause lenv fg sk fk cutk)
...
(define (prove-goal goal lenv fg sk fk)
  (let ([clauses (get-clauses (car goal) (length (cdr goal)))])
    (cond
      [(null? clauses)
       (error 'prove-goal "predicate ~a/~a not defined" (car goal) (length (cdr goal)))]
      [(procedure? clauses)
       (apply clauses lenv fg sk fk (cdr goal))]
      [else
       (try-each goal clauses lenv fg sk fk)])))

(define (prove-goal-lst goal-lst lenv fg sk fk cutk)
  (cond
    [(null? goal-lst) (sk lenv fg fk)]
    [(eq? '! (car goal-lst))
     (prove-goal-lst (cdr goal-lst) lenv fg sk cutk cutk)]
    [(null? (cdr goal-lst))
     (prove-goal (car goal-lst) lenv fg sk fk)]
    [else
     (prove-goal (car goal-lst)
                 lenv
                 fg
                 (lambda (lenv2 fg2 fk2)
                   (prove-goal-lst (cdr goal-lst) lenv2 fg2 sk fk2 cutk))
                 fk)]))
...
(add-primitive 'call 1
               (lambda (lenv fg sk fk goal)
                 (let ([new-goal (must-deref goal lenv 'call/1)])
                   (prove-goal new-goal lenv fg sk fk))))
...
(add-primitive 'write 1
               (lambda (lenv fg sk fk term)
                 (printf "~a" (simplify-term term lenv))
                 (sk lenv fg fk)))

(add-primitive 'nl 0
               (lambda (lenv fg sk fk)
                 (newline)
                 (sk lenv fg fk)))
...
(add-primitive 'copy 2
               (lambda (lenv fg sk fk src-term new-term)
        ...
        (cond
          ...
          [else
           ...
           (sk new-lenv
               fg
               (lambda ()
                 (retract-impl lenv fg sk fk head)))])))))
...
(define (awaken-fg new-lenv old-lenv fg sk fk)
...
(add-primitive 'freeze 2
               (lambda (lenv fg sk fk var goal)
                 (let ([val (deref var lenv)])
                   (if (var? val)
                       (sk lenv (cons (cons val `(freeze ,val ,goal)) fg) fk)
                 ...
                 (let ([val (deref var lenv)])
                   (if (not (eq? var val))
                       (let ([new-goal (must-deref goal lenv 'delay/2)])
                         (prove-goal new-goal lenv fg sk fk))
                       (sk lenv (cons (cons var goal) fg) fk)))))
...
(define (try-dif a b s cs)
  (let ([new-s (unify a b s)])
    (cond
      [(not new-s) cs]
      [(eq? s new-s) #f]
      [else
       (cons (prefix-bindings new-s s) cs)])))

(define (run-dif a b)
  (let ([new-cs (try-dif a b s cs)])
    (cond
      [(eq? cs new-cs) 'succeeded]
      [(not new-cs)
       (set! s #f)
       (set! cs #f)
       'failed]
  ...
  (if (null? cs)
      ...
      (cond
        [(unify* (car cs) s)
         =>
         (lambda (result)
           (let ([new-c (prefix-bindings result s)])
             (if (null? new-c)
                 #f
                 (let ([rest (rerun-constraints (cdr cs) s)])
                   (if rest
                       (cons new-c rest)
                       #f)))))]
        [else
         (rerun-constraints (cdr cs) s)])))
...
(define (run-eq a b)
  (let ([new-s (unify a b s)])
    (cond
      [(not new-s) 'failed]
      [else
       (if (null? (prefix-bindings new-s s))
           'succeeded
           (let ([new-cs (rerun-constraints cs new-s)])
             (cond
               [new-cs
                (set! s new-s)
                (set! cs new-cs)
