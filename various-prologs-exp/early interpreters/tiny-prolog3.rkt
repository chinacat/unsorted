...
(define (select-frozen-goals lenv lst)
  (if (null? lst)
      (values '() '())
      (let-values ([(rst-awake rst-freeze)
                    (select-frozen-goals lenv (cdr lst))])
        (if (can-awaken? (caar lst) lenv)
...
(define (awaken-fg new-lenv old-lenv fg sk fk)
  (let-values ([(goals new-fg) (select-frozen-goals new-lenv fg)])
    (if (null? goals)
        (sk new-lenv fg fk)
...
(add-primitive 'freeze 2
               (lambda (lenv fg sk fk var goal)
                 (let ([val (deref var lenv)])
                   (if (var? val)
                       (sk lenv (cons (cons var `(freeze ,var ,goal)) fg) fk)
                       (let ([new-goal (must-deref goal lenv 'freeze/2)])
                         (prove-goal new-goal lenv fg sk fk))))))

(add-primitive 'delay 2
               (lambda (lenv fg sk fk var goal)
                 (if (can-awaken? var lenv)
