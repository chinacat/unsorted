...
;(add-primitive 'different 3
;               (lambda (lenv fg sk fk t1 t2 result)
;                 (let ([new-lenv (unify t1 t2 lenv)])
;                   (cond
;                     [(eq? new-lenv #f)
;                      (let ([result-lenv (unify result 'true lenv)])
;                        (if result-lenv
;                            (sk result-lenv fg fk)
;                            (fk)))]
;                     [(eq? lenv new-lenv)
;                      (let ([result-lenv (unify result 'false lenv)])
;                        (if result-lenv
;                            (sk result-lenv fg fk)
;                            (fk)))]
;                     [else
;                      (let ([result-lenv (unify result `(maybe ,(caar new-lenv)) lenv)])
;                        (if result-lenv
;                            (sk result-lenv fg fk)
;                            (fk)))]))))
               ...
               (lambda (lenv fg sk fk t1 t2)
                 (let ([new-lenv (unify t1 t2 lenv)])
                   (cond
                     ...
                     [else
                      (sk lenv
                          (cons (cons (caar new-lenv) `(dif ,t1 ,t2)) fg)
                          fk)]))))
