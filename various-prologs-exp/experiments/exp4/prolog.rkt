    ...
    (cond
      ...
      [(null? (cdr clauses))
       (apply (car clauses)
              ...
              args)]
      [else
       (let ([choice-point (make-fk sk fk (cdr clauses) args *trail*)])
...
(define (try-again sk parent-fk clauses args old-trail)
  ...
  (cond
    ...
    [else
     (apply (car clauses)
            sk
            parent-fk
            (make-fk sk parent-fk (cdr clauses) args old-trail)
            args)]))
    ...
    [(_ g ...)
     (let-values ([(ngs nvs) (compute-goals (syntax->list #'(g ...)) '() '())])
       (with-syntax ([(ng ...) ngs]
                     [(var ...) (reverse nvs)])
         #`(let/ec exit
             (let ([fail (make-top-fk exit)]
                   [var (make-variable 'var #f *unbound*)] ...)
               (compile-goals (make-top-sk fail exit (list var ...))
                              fail
                              fail
                              (ng ...))))))]))

(define (make-fk sk parent-fk clauses args old-trail)
  (set! count-fk (+ count-fk 1))
  (lambda ()
    (try-again sk parent-fk clauses args old-trail)))

(define (make-sk sk cutk goals)
  (set! count-sk (+ count-sk 1))
  (lambda (fk)
    (execute-goals sk cutk fk goals)))

(define (make-top-fk exit)
  (lambda ()
    (reset-trail '())
    (printf "~nNo.")
    (exit)))

(define (make-top-sk fail exit vars)
  (lambda (fk)
...
(define (make-benchmark-sk fail exit vars)
  (lambda (fk)
    (show-prolog-vars vars)
    (cond
      [(eq? fail fk)
       ...
       (with-syntax ([(ng ...) ngs]
         ...
         #`(begin (reset-counts)
                  (time (let/ec exit
                          (let ([fail (make-top-fk exit)]
                                [var (make-variable 'var #f *unbound*)] ...)
                            (execute-goals (make-benchmark-sk fail exit (list var ...))
                                           fail
                                           fail
