  ...
  (cond
    ...
    [else
     (apply (car clauses)
            sk
            parent-fk
            (build-fk sk parent-fk (cdr clauses) args old-trail)
            args)]))
...
(define-struct failk (sk parent-fk clauses args old-trail)
  #:mutable
  #:transparent)

(define (build-fk sk parent-fk clauses args old-trail)
  (set! count-fk (+ count-fk 1))
  (make-failk sk parent-fk clauses args old-trail))
