...
(define-syntax compile-goals
  (syntax-rules (!)
    ...
    [(_ sk cutk fk (g0 g ...))
     (let ([goal (transform-term g0)])
       (dispatch (car goal)
                 (lambda (fk)
                   (compile-goals sk cutk fk (g ...)))
                 fk
                 (cdr goal)))]))
...
(define-struct failk (sk parent-fk clauses args old-trail next)
  #:mutable
  #:transparent)
...
(define (build-fk sk parent-fk clauses args old-trail)
  (if (failk? *recycle-list*)
      ...
      (begin (set! count-fk (+ count-fk 1))
             (make-failk sk parent-fk clauses args old-trail #f))))

(define (build-sk sk cutk goals)
  (printf "in build-sk~n")
