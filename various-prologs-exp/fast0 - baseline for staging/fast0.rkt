...
(define (try-each clause other-clauses sk fk arg)
  (let ([frame (make-vector (clause-frame-size clause))])
    (let ([old-trail *trail*])
      (cond
        [((make-unify-head (cdr (clause-head clause))) arg frame)
        ...
        [(pair? other-clauses)
         (reset-trail old-trail)
         (try-each (car other-clauses) (cdr other-clauses) sk fk arg)]
        [else
         (fk)]))))

(define (prove-goal goal top-level? frame sk fk)
  (let ([predicate-name (car goal)]
        [arg (cdr goal)])
    (let ([copied-arg (if top-level?
                          arg
                          ((make-copy-term arg) frame))]
          [clauses (get-clauses predicate-name)])
      (cond
        [(null? clauses)
         (error 'prove-goal "predicate ~a not defined" predicate-name)]
        [(procedure? clauses)
         (apply clauses sk fk copied-arg)]
        [else
         (try-each (car clauses) (cdr clauses) sk fk copied-arg)]))))

(define (prove-goal-lst goal-lst top-level? frame sk fk cutk)
  (if (null? goal-lst)
      (sk fk)
      (let ([goal (car goal-lst)]
        ...
        (cond
          [(eq? '! goal)
           (prove-goal-lst others top-level? frame sk cutk cutk)]
          [(null? others)
           (prove-goal goal top-level? frame sk fk)]
          [else
           (prove-goal goal
                       top-level?
                       frame
                       (lambda (fk2)
                         (prove-goal-lst others top-level? frame sk fk2 cutk))
                       fk)]))))
