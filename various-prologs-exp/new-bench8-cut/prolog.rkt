...
(define-syntax (gen-goal-seq stx)
  (syntax-case stx (!)
    ...
    [(_ sk fk cutk (g))
     #'(gen-call sk fk g)]
    [(_ sk fk cutk (! g0 g ...))
     #'(gen-goal-seq sk cutk cutk (g0 g ...))]
    [(_ sk fk cutk (g0 g ...))
     #'(gen-call (lambda (fk2) (gen-goal-seq sk fk2 cutk (g ...)))
                 fk
                 g0)]))

(define-for-syntax (can-cut? goals)
  (ormap (lambda (term) (eq? '! (syntax-e term)))
          (syntax->list goals)))

(define-syntax (gen-body-seq stx)
  (syntax-case stx ()
    [(_ sk fk ck ())
     #'(sk fk)]
    [(_ sk fk ck (g0 g ...))
     (if (can-cut? #'(g0 g ...))
         #'(let ([cutk ck])
             (gen-goal-seq sk fk cutk (g0 g ...)))
         #'(gen-goal-seq sk fk #f (g0 g ...)))]))
     ...
     (let ([src-vars (collect-src-vars #'(term0 term ...))])
       (let-values ([(new-term-lst vars)
         ...
         (with-syntax ([(g ...) new-term-lst]
                       [(v ...) vars])
           #`(let ([v #f] ...)
               (gen-goal-seq top-sk top-fk top-fk (g ...))))))]))
...
(define bench-samples 300)
(define strip-count 0)
...
(define (benchmark-time* thunk count)
  ...
  (let* (;[timings (sort (for/list ([i (in-range count)])
         ...
         ;                        (collect-garbage)))
         ;              <)]
         [timings (for/list ([i (in-range count)])
                    (begin0 (cpu-time thunk)
                            (collect-garbage)
                            (collect-garbage)
                            (collect-garbage)))]
         [useful (drop (drop-right timings strip-count) strip-count)])
    (values timings
            (exact->inexact (/ (for/sum ([i useful]) i) (length useful))))))
