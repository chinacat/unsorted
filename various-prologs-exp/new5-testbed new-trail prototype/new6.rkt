...
; new6 -- EXPERIMENT, with a vector-based trail. Current version is one 'big' (1000) sized vector
;
; TO DO:
...
(define *trail-vector* (make-vector 1000))
(define *trail* 0)
...
(define (reset-trail old)
  (cond
    [(> *trail* old)
     (set! *trail* (- *trail* 1))
     (set-var-val! (vector-ref *trail-vector* *trail*) *unbound*)
     (reset-trail old)]
    [else
     (void)]))
...
(define (set-binding! var val)
  (unless (eq? var val)
    (vector-set! *trail-vector* *trail* var)
    (set! *trail* (+ *trail* 1))
    (set-var-val! var val))
  #t)
...
(define (try-each goal clauses sk fk)
  (cond
    [(null? (cdr clauses))
     (try-goal goal
               (car clauses)
     ...
     {let ([old-trail *trail*])
       (try-goal goal
                 ...
                 (lambda ()
                   ...
                   (try-each goal (cdr clauses) sk fk))
                 fk)}]))

(define (try-goal goal clause sk fk cutk)
...
(define-syntax benchmark
  (syntax-rules ()
    [(_ g0 g ...)
     (begin (time (top-level-solve '(g0 g ...) #t)))]))
