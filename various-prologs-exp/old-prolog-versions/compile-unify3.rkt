...
; To do:
; [8/22/2012]
; 1. Update ?- to support multiple goals
; 2. Add support of cut
...
(define-syntax (compile-clause stx)
  ...
  (define (compute-goal goal vars)
    (let-values ([(new-goal new-vars) (walk goal vars)])
      (with-syntax ([(functor arg ...) new-goal])
  ...
  (define (compute-goals in-goals acc-goals vars)
    (if (null? in-goals)
  ...
  (syntax-case stx ()
    [(_ head body)
     (let*-values ([(new-head vars0) (walk #'head '())]
...
(define (execute-goals sk fk goals)
  (cond
    [(null? goals)
     (sk fk)]
    [else
     (let ([goal (car goals)])
       (apply dispatch
              (car goal)
              (lambda (fk2)
                (execute-goals sk fk2 (cdr goals)))
              fk
              (cdr goal)))]))
...
(define-syntax prepare-query-goal
  (syntax-rules ()
    [(_ predname (var ...) (arg ...))
     (let/ec exit
       (let ([fail (lambda ()
  ...
  (syntax-case stx ()
    [(_ predname (arg ...))
     (let*-values ([(new-args vars) (walk #'(arg ...) '())])
       #`(prepare-query-goal predname #,vars #,new-args))]))
