  ...
  (let ([pred (predicate (clause-head clause))])
    ...
    (let ([existing-clauses (get-clauses pred)])
      (if (procedure? existing-clauses)
          ...
          (putprop pred 'clauses (list clause))
...
(define *count-instantiations* 0)

(define (instantiate x activation-frame)
  (set! *count-instantiations* (+ *count-instantiations* 1))
  (cond
...
(define (top-level-prove goals continue?)
  (let/ec exit
    (let ([fail (lambda ()
                  (reset-trail '())
                  (printf "~nNo.")
                  (printf "instantiations=~a~n" *count-instantiations*)
                  (exit))])
      (prove-query goals
                   (lambda (failure-cont)
...
(define (analyze-clause clause)
  (let ([args (cdr (clause-head clause))]
        ...
        [copy-size (length (clause-body clause))]
    ...
    (lambda (goal-args success-cont cut-cont failure-cont)
      (let ([new-frame (new-activation-frame num-vars
        ...
        (if (unify! goal-args (instantiate args new-frame))
            (prove-goals body
                         new-frame
                         copy-stack
                         0
