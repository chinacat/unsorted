...
(define *count-var-set-ops* 0)
(define (count-var-set)
  (set! *count-var-set-ops* (+ *count-var-set-ops* 1)))

(define *trail-records* '())
(define (record-trail-length)
  (set! *trail-records* (cons (length *trail*) *trail-records*)))

(define (statistics)
  (define sorted-records (sort *trail-records* >))
  (printf "~n~nStatistics:~n")
  (printf "Number of var set ops: ~s~n" *count-var-set-ops*)
  (printf "Number of trail reset ops: ~s~n" (length *trail-records*))
  (printf "Max trail length: ~s~n" (car sorted-records)))
...
(define-syntax ?-
  (syntax-rules ()
    [(_ goal0 goal ...)
     (begin (top-level-prove (transform-query-vars goal0 goal ...))
            (statistics))]))
...
(define-for-syntax (compile-fact stx)
  (let-values ([(new-head functor-bindings var-bindings)
                (walk stx '() '())]
               [(functor arity)
                (syntax-case stx ()
                  [(f arg ...)
                   (values #'f (length (syntax->list #'(arg ...))))])])
    #`(let #,(map (lambda (binding)
                    (list (cdr binding)
                          #`(intern-functor (quote #,(caar binding))
                                            (quote #,(cdar binding)))))
                      ...
                      (let #,(map (lambda (binding)
                                    (list (cdr binding)
                                          #`(make-variable (quote #,(car binding))
                                                           (new-counter))))
                                  (reverse var-bindings))
                        (if (unify! goal #,new-head)
                            (success-cont failure-cont)
                            (failure-cont))))))))

(define-for-syntax (compile-rule head body)
  (let*-values ([(new-head head-functor-bindings head-var-bindings)
                 (walk head '() '())]
                [(new-body functor-bindings var-bindings)
                 (walk body head-functor-bindings head-var-bindings)]
                [(functor arity)
                 (syntax-case head ()
                   [(f arg ...)
                    (values #'f (length (syntax->list #'(arg ...))))])])
