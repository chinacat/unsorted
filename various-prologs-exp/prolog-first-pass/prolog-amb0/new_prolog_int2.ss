...
(define-struct choices (k goal old-trail alts)
  #:mutable)
  ...
  (if (null? stack)
      ...
      (let* ([frame (car stack)]
        ...
        (let ([first (car thunks)]
          ...
          ((choices-k frame) (first goal cut-stack))))))

(define (prune-choices cut-stack)
  (set! stack cut-stack))
                          ...
                          (let #,(map (lambda (binding)
                                        (list (cdr binding)
                                              #`(make-variable (quote #,(car binding))
                                                               (new-counter))))
                                      (reverse var-bindings))
                            (unless (unify! (cdr goal) #,new-args)
                              (amb-fail)))))))))
...
; prove-goals : goal-list cut-cont -> void
(define (prove-goals goal-list cut-cont)
  (cond
    [(eq? '! (car goal-list))
     (prune-choices cut-cont)
     (if (null? (cdr goal-list))
         (void)
         (prove-goals (cdr goal-list) cut-cont))]
    [(null? (cdr goal-list))
  ...
  (let ([clauses (predicate-clauses (functor goal))])
    (cond
      ...
      [(null? clauses)
       (amb-fail)]
      [(null? (cdr clauses))
       ((car clauses) goal stack)]
      [else
       (call/cc (lambda (k)
                  (let ([first (car clauses)]
                        [rest (cdr clauses)])
                    (let ([new-frame (make-choices k goal *trail* rest)])
                      (set! stack (cons new-frame stack))
                      (first goal (cdr stack)))))
                amb-prompt-tag)])))

(define (top-level-prove goals)
  (let ([var-list (variables-in goals)])
...
(add-primitive '= 2
               (lambda (args)
                 (unless (unify! (car args) (cadr args))
                   (amb-fail))))

;(add-primitive 'member 2
;               (letrec ([mem-proc (lambda (args)
;                                    (let ([?arg1 (car args)]
;                                          [?arg2 (cadr args)])
