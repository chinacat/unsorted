...
(define *trail* (make-vector 5000 #f))
(define trail-next 0)

(define (reset-trail old)
  (when (< old trail-next)
    (let iter ()
      (set! trail-next (- trail-next 1))
      (set-variable-val! (vector-ref *trail* trail-next) *unbound*)
      (vector-set! *trail* trail-next #f)
      (when (< old trail-next)
        (iter)))))
...
(define (set-binding! var val)
  (unless (eq? var val)
    (vector-set! *trail* trail-next var)
    (set! trail-next (+ trail-next 1))
