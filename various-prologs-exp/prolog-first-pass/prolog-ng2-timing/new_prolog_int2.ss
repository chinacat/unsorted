...
(define unify-count 0)
(define total-cpu 0)
(define total-real 0)
(define total-gc 0)

(define (unify-statistics)
  (printf "Unify attempt statistics:~n")
  (printf "total attempts = ~s~n" unify-count)
  (printf "total cpu time = ~s~n" total-cpu)
  (printf "total real time = ~s~n" total-real)
  (printf "total gc time = ~s~n" total-gc)
  (newline))

(define (unify! x y)
  (let-values ([(results cpu real gc) (time-apply *unify! (list x y))])
    (set! unify-count (+ unify-count 1))
