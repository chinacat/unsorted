...
(define (prove-goals goal-list success-cont)
  (cond
    ;[(eq? '! (car goal-list))
    ...
    [else (search-choices (car goal-list)
                          (lambda ()
                            (prove-goals (cdr goal-list) success-cont)))]))
      ...
      [else
       (let ([old-trail *trail*])
         (let iter ([clauses clauses])
           (if (null? (cdr clauses))
               ((car clauses) goal success-cont)
               (begin ((car clauses) goal success-cont)
