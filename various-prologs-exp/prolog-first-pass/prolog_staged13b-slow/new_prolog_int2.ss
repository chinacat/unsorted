...
(define (constant-term? x)
  (cond
    [(or (variable-template? x) (anonymous-variable-template? x))
     #f]
    [(pair? x)
     (and (constant-term? (car x)) (constant-term? (cdr x)))]
    [else
     #t]))

(define (make-copier x)
  ;activation-frame)
  (cond
    [(variable-template? x)
     (let ([index (variable-template-index x)])
       (lambda (activation-frame)
         (vector-ref activation-frame index)))]
    [(anonymous-variable-template? x)
     (lambda (activation-frame)
    ...
    [(pair? x)
     (cond
       [(null? (cdr x))
        (let ([copy-car (make-copier (car x))])
          (lambda (activation-frame)
            (cons (copy-car activation-frame) '())))]
       [(constant-term? (car x))
        (let ([car-term (car x)]
              [copy-cdr (make-copier (cdr x))])
          (lambda (activation-frame)
            (cons car-term (copy-cdr activation-frame))))]
       [(constant-term? (cdr x))
        (let ([cdr-term (cdr x)]
              [copy-car (make-copier (car x))])
          (lambda (activation-frame)
            (cons (copy-car activation-frame) cdr-term)))]
       [else
        (let ([copy-car (make-copier (car x))]
              [copy-cdr (make-copier (cdr x))])
          (lambda (activation-frame)
            (cons (copy-car activation-frame)
                  (copy-cdr activation-frame))))])]
    [else
     (lambda (activation-frame)
       x)]))
...
(define (memo-instantiate goal-copier new-frame copy-stack index)
  (let ([val (vector-ref copy-stack index)])
    (if (eq? val *unbound*)
        (let ([new-val (goal-copier new-frame)])
          (vector-set! copy-stack index new-val)
          new-val)
        val)))
...
(define (prove-goals goal-list new-frame copy-stack index success-cont cut-cont failure-cont)
  (cond
    ...
    [(null? (cdr goal-list))
     (let ([goal (car goal-list)])
       (search-choices (car goal)
                       (memo-instantiate (make-copier (cdr goal)) new-frame copy-stack index)
                       success-cont
                       failure-cont))]
    [else (let ([goal (car goal-list)])
            (search-choices (car goal)
