...
(define (prove-query goal-list bindings success-cont cut-cont failure-cont)
  (cond
    ...
    [(eq? '! (car goal-list))
     (if (pair? (cdr goal-list))
         (prove-query (cdr goal-list) bindings success-cont cut-cont cut-cont)
...
(define (prove-goals goal-list new-frame bindings success-cont cut-cont failure-cont)
  (cond
    [(null? goal-list)
     (success-cont bindings failure-cont)]
    [(eq? '! (car goal-list))
     (if (pair? (cdr goal-list))
         (prove-goals (cdr goal-list) new-frame bindings success-cont cut-cont cut-cont)
         (success-cont cut-cont))]
    [(null? (cdr goal-list))
    ...
    [else (search-choices (instantiate (car goal-list) new-frame)
                          bindings
                          (lambda (b2 fc2)
                            (prove-goals (cdr goal-list) new-frame b2 success-cont cut-cont fc2))
                          failure-cont)]))
...
(define (analyze-clause clause)
  (let ([args (cdr (clause-head clause))]
    ...
    (lambda (goal-args bindings success-cont cut-cont failure-cont)
      (let ([new-frame (new-activation-frame num-vars
                                             var-vector)])
        (let ([new-bindings (unify goal-args (instantiate args new-frame) bindings)])
          (if new-bindings
              (prove-goals body
                           new-frame
                           new-bindings
