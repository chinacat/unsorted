...
(define (prove-goal-lst lenv sk fk)
  (cond
    [(null? goal-lst-reg)
     (set! sk-reg sk)
     (set! lenv-reg lenv)
    ...
    [else
     ...
     (set! lenv-reg lenv)
     (set! sk-reg (goal-sk (cdr goal-lst-reg) sk))
     (set! fk-reg fk)
     (prove-goal)]))

(define (top-level-solve goal-lst)
  (set! goal-lst-reg goal-lst)
  (prove-goal-lst '() (top-sk (reverse (collect-vars goal-lst '()))) (top-fk)))
...
(define (invoke-sk)
  (match sk-reg
    [(goal-sk goals sk)
     ...
     (prove-goal-lst lenv-reg sk fk-reg)]
    [(top-sk user-vars)
     (printf "solved: ~n")
     (for-each (lambda (var)
