              ...
              (cond
                [new-lenv
                 (let ([saved-lenv lenv-reg])
                   ...
                   (set! fk-reg (choice-point goal-reg (cdr clauses-reg) saved-lenv sk-reg fk-reg))
                   (prove-goal-lst))]
                [else
...
(define (prove-goal-lst)
  (cond
    ...
    [else
     ...
     (set! sk-reg (goal-sk (cdr goal-lst-reg) sk-reg))
     (prove-goal)]))

(define (top-level-solve goal-lst)
  (set! goal-lst-reg goal-lst)
  (set! sk-reg (top-sk (reverse (collect-vars goal-lst '()))))
  (set! fk-reg (top-fk))
  (set! lenv-reg '())
  (prove-goal-lst))
