  ...
  (cond
    ...
    [else (let ([new-clause (copy-clause (car clauses-reg))])
            (let ([new-lenv (unify goal-reg (car new-clause) lenv-reg)])
              (cond
                [new-lenv
                 (let ([saved-lenv lenv-reg])
                   (set! goal-lst-reg (cdr new-clause))
                   (set! lenv-reg new-lenv)
                   (set! fk-reg (build-choice-point goal-reg (cdr clauses-reg) saved-lenv sk-reg fk-reg))
                   (prove-goal-lst))]
                [else
...
(define (prove-goal-lst)
  (cond
    [(null? goal-lst-reg)
     (invoke-sk)]
    [else
     (set! goal-reg (car goal-lst-reg))
     (set! sk-reg (build-goal-sk (cdr goal-lst-reg) sk-reg))
     (prove-goal)]))

(define (top-level-solve goal-lst)
  (set! goal-lst-reg goal-lst)
  (set! sk-reg (build-top-sk (reverse (collect-vars goal-lst '()))))
  (set! fk-reg (build-top-fk))
  (set! lenv-reg '())
  (prove-goal-lst))

(struct top-fk ())
(struct choice-point (goal clauses lenv sk fk) #:transparent #:mutable)
(define build-top-fk top-fk)
(define build-choice-point choice-point)
...
(define build-top-sk top-sk)
(define build-goal-sk goal-sk)

(define (invoke-sk)
  (match sk-reg
    [(goal-sk goals sk)
     (set! goal-lst-reg goals)
     (set! sk-reg sk)
     (prove-goal-lst)]
    [(top-sk user-vars)
     (printf "solved: ~n")
