    ...
    (if (or (pair? temp4) (var? temp4))
        (let ([?x (if (pair? temp4)
              ...
              [?rest (if (pair? temp4)
                         ...
                         (var '?rest *unbound*))])
          (when (var? temp4)
            (set! count-temp4-is-var (+ count-temp4-is-var 1))
...
(define count-temp-is-var 0)
(define count-rst-is-var 0)
    ...
    (if (or (pair? temp) (var? temp))
        (let ([?x (if (pair? temp)
              ...
              [?rst (if (pair? temp)
                        ...
                        (var '?rst *unbound*))])
          (when (var? temp)
            (set! count-temp-is-var (+ count-temp-is-var 1))
                              ...
                              (if (unify! ?left ?x)
                                  (if (let ([?rst (deref ?rst)])
                                        (if (pair? ?rst)
                                            ...
                                            (if (var? ?rst)
                                                (begin 
                                                  (set! count-rst-is-var (+ count-rst-is-var 1))
