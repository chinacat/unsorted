...
; v2 -- head is now a template (the result of pre)
(define (unify-head head arg frame subst)
  (cond
    [(null? head)
     (cond
       [(null? arg) subst]
       [(var? arg) (unify-var-to-term arg '() subst)]
       [else #f])]
    [(set-var? head)
    ...
    [(not (pair? head))
     (cond
       [(equal? head arg) subst]
       [(var? arg) (unify-var-to-term arg head subst)]
       [else #f])]
    [(pair? head)
     (cond
       [(pair? arg)
       ...
       [(var? arg) (unify-var-to-term arg (copy-term head frame) subst)]
       [else #f])]
    [else (error 'unify-head "unexpected head datum, given ~a" head)]))
