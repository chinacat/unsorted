...
; v4 -- curry head vs. arg+frame+subst, as first step towards staging
...
'((make-unify-head (list 'q (list 'p (set-var '?x 0) (set-var '?y 1)) (list 'p (ref-var '?y 1) (ref-var '?x 0))))
  '(q ?z ?z)
  (make-vector 2)
  '())
...
'(pre '(?a (1 ?x 2 ?y 3) ?b))
'((make-unify-head (list (set-var '?a 0) (list 1 (set-var '?x 1) 2 (set-var '?y 2) 3) (set-var '?b 3)))
  '(55 ?z 66)
  (make-vector 4)
  '())

'(pre '(?a ?b ?c ?b ?a))
'((make-unify-head (list (set-var '?a 0) (set-var '?b 1) (set-var '?c 2) (ref-var '?b 1) (ref-var '?a 0)))
  '(1 (2 ?a) 3 (?z 5) 1)
  (make-vector 3)
  '())
