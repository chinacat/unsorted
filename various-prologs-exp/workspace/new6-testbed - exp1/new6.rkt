...
; EXPERIMENTS
; exp1 = replace the specialized unify with a specialized copier for the head + a call to unify!
;;       437 ms vs. 1390 ms
...
(define (intern-clause new-clause-source)
  (let-values ([(pre-clause frame-size) (pre (map rewrite-goal new-clause-source))])
    (clause (make-copy-term (car pre-clause))
    ...
    (let ([head (clause-head clause)]
      ...
      (let ([old-trail *trail*])
        (cond
          ...
          [(pair? other-clauses)
           (reset-trail old-trail)
           (try-each goal other-clauses sk fk)]
          [else
           (fk)])))))
